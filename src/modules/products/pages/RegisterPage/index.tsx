...
const ProductsRegisterPage: React.FC = () => {
  ...
  const [fields, setFields] = useState<any[]>([
    ...
    {
      field:
        'DESEJA DESCREVER OS FATOS OCORRIDOS NO DIA DA AUTUAÇÃO? APONTAR ALGUMA IRREGULARIDADE?',
      type: 'SELECT',
      options: [
        {
    ...
    {
      field:
        'RECEBEU A NOTIFICAÇÃO PADRÃO DO DETRAN, OU POR MEIO DO SISTEMA NACIONAL DE NOTIFICAÇÃO ELETRÔNICA?',
      type: 'SELECT',
      options: [
        {
          value: 'SIM, RECEBI. ',
    ...
    {
      field:
        'A PRIMEIRA NOTIFICAÇÃO FOI POSTADA DENTRO DOS 30 DIAS DO COMETIMENTO DA INFRAÇÃO?',
      type: 'SELECT',
      options: [
        {
    ...
    {
      field:
        'A AUTUAÇÃO POSSUI FOTO DO VEÍCULO OU A DESCRIÇÃO DO VEÍCULO DIFERENTE DO SEU?',
      type: 'SELECT',
      options: [
        {
          value:
            'Não, as informações na multa estão iguais ao documento do meu veículo. ',
          anexo: '',
          relation: ''
        },
        {
    ...
    {
      field:
        'O CAMPO DAS OBSERVAÇÕES DA NOTIFICAÇÃO FOI PREENCHIDO DESCREVENDO OS FATOS? ',
      type: 'SELECT',
      options: [
        {
    ...
    {
      field:
        'O CONDUTOR É REINSCIDENTE NA MESMA INFRAÇÃO NOS ÚLTIMOS 12 MESES (LEMBRE-SE DE VERIFICAR SE É O MESMO ARTIGO, E A MESMA FICHA DE AUTUAÇÃO)?',
      type: 'SELECT',
      options: [
        {
          value: 'SIM. ',
    ...
    try {
      ...
      apiBackend
        .post('/resources', {
          ...data,
          photo,
          value: String(data?.value).replace(/\./g, '').replace(/,/, '.'),
    ...
    <Form ref={formRef}>
        ...
        >
          ...
          <Flex width="100%" justifyContent={'space-between'}>
            <Flex width={['100%', '100%', '48%']} flexDirection="column">
              ...
              <Flex width={'100%'} flexDirection="column">
                {photo && <Image src={photo} width="200px" height="200px" />}
                <Input
                  name="photo"
              ...
              <Flex width={['100%']} justifyContent="space-between">
                <Flex width="48%">
                  <Input
                    ...
                    label="Artigo"
                    onKeyUp={e =>
                      handleChangeField('CODIGO_ARTIGO', e.currentTarget.value)
                    }
                  />
                </Flex>
                <Flex width="48%">
                  <Input
                    ...
                    label="Cód. Ficha"
                    onKeyUp={e =>
                      handleChangeField('CODIGO_FICHA', e.currentTarget.value)
                    }
                  />
                </Flex>
              </Flex>
              ...
              <Flex width={'100%'}>
                <TextArea name="description" label="Descrição (Motivo)" />
              </Flex>
              <Flex width={'48%'}>
                <InputCurrency name="value" size="sm" label="Preço" />
              </Flex>
            </Flex>
          </Flex>
          ...
          <Box width="100%" flexDirection="column">
            <Flex
              ...
              p="16px"
            >
              <Text fontWeight="600" fontSize="14px" color="green.500">
                Flag Personalizada
              </Text>

              <Text fontSize="12px" fontWeight="500" color="gray.500">
                Se o recurso precisar de uma informação específica, crie uma
                flag customizada para usar no documento.
              </Text>
              ...
              <Flex width="100%" justifyContent="space-between" mt="8px">
                <Flex width="48%" flexDirection="column">
                  ...
                  <Flex width="100%" flexDirection="column">
                    <Select
                      ...
                      label="Tipo de campo"
                      onChange={e => {
                        setCurrentFieldType(e.currentTarget.value);
                        handleResetOptions();
                      }}
                ...
                >
                  <Flex width="100%" flexDirection="column">
                    <Box fontWeight="500">Relacionamento</Box>
                    <Flex width="100%" gap="8px">
                      <Flex alignItems="center" fontSize="12px" gap="4px">
                        <input
                          ...
                          value="PARAGRAPH"
                          name="relation"
                          checked={relationField === 'PARAGRAPH'}
                          onChange={e =>
                            setRelationField(e.currentTarget.value)
                          }
                        />
                        <Text>Paragráfo</Text>
                      </Flex>
                      <Flex alignItems="center" fontSize="12px" gap="4px">
                        <input
                          ...
                          value="OPEN_FIELD"
                          name="relation"
                          checked={relationField === 'OPEN_FIELD'}
                          onChange={e =>
                            setRelationField(e.currentTarget.value)
                          }
                        />
            ...
            <Flex width="100%" flexDirection="column" fontSize="14px" mb="16px">
              <Text fontWeight="600" color="green.500">
                Modo de usar
              </Text>
              <Text>
                1 - Clique no campo desejado abaixo para copiar sua flag e
                colocar no documento
              </Text>
              ...
              <Text>
                3 - As flags serão substituidas posteriormente pelos valores
                informado pelo comprador do recurso
              </Text>
            </Flex>
            <Flex flexDirection="column" fontSize="14px" mb="8px">
              ...
              <Flex width="100%" gap="8px" mt="8px" flexWrap="wrap" mb="8px">
                {fields.map(item => (
                  <Flex
                    key={item.flag}
                    color="white"
                  ...
                  >
                    <Flex
                      cursor="pointer"
                      onClick={() => handleCopyFlag(item.flag)}
                      alignItems="center"
                      justifyContent="center"
                    >
                      <Text lineHeight="14px">
                        {String(item.field).substring(0, 32)}
                      </Text>
                    ...
                    <Flex position="absolute" left="-4px" top="-10px" gap="4px">
                      {!item?.default &&
                        item.flag !== '{{ ORGAO_AUTUADOR }}' && (
                          <Flex
                            p="2px"
                            borderRadius="4px"
                            backgroundColor="red.500"
                            color="white"
                            cursor="pointer"
                            onClick={() => handleDeleteFlag(item.flag)}
                          >
                            <Icon as={FiX} color="white" fontSize="14px" />
                          </Flex>
                        )}
                      ...
                      {item?.edit && (
                        <Flex
                          ...
                          color="white"
                          cursor="pointer"
                          onClick={() => handleEditFlag(item.flag)}
                        >
                          <Icon as={FiEdit} color="white" fontSize="12px" />
                        </Flex>
                      )}
                    </Flex>
                  </Flex>
              ...
              {fields.length <= 0 && (
                <Text>Você não criou nenhum campo personalizado ainda</Text>
              )}
            </Flex>
      ...
      {currentEditFlag && (
        <ModalEditFlag
          ...
          onChange={(flag, data) => handleChangeOptionsFlag(flag, data)}
        />
      )}
    </Form>
  );
